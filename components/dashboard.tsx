...
export function Dashboard() {
  ...
  return <main className="shell">
    <aside className="sidebar">
      <a className="brand" href="#"><span className="brand-mark">♛</span> ListingKing</a>
      ...
      <div className="sidebar-section-label">Create listing</div>
      <div className="sidebar-steps">
        {stages.map((name, index) => (
          <button key={name} onClick={() => index <= step && setStep(index)} className={`sidebar-step ${index === step ? "current" : index < step ? "done" : ""}`}>
            <b>{index < step ? "✓" : String(index + 1).padStart(2, "0")}</b>
            <span>{name}<small>{stageSubtitles[index]}</small></span>
          </button>
        ))}
      </div>
      <div className="seller"><span className="avatar">KB</span><div><strong>Kunj Balar</strong><small>kunj@example.com</small><button className="signout" onClick={() => signOut({ callbackUrl: "/sign-in" })}>Sign out</button></div></div>
    </aside>
    <section className="content">
      <header className="topbar">
        <div><span className="breadcrumb">Create listing</span> <span className="breadcrumb-sep">/</span> <span className="breadcrumb">{stages[step]}</span></div>
        <div className="secure"><i /> Meesho connection <span className="secure-chevron">▾</span></div>
      </header>
      <div className="stepper">{stages.map((name, index) => <button key={name} onClick={() => index <= step && setStep(index)} className={index === step ? "current" : index < step ? "done" : ""}><b>{index < step ? "✓" : index + 1}</b><span>{name}<small>{stageSubtitles[index]}</small></span></button>)}</div>
      ...
      <footer className="footer"><span className="footer-status">{step === 0 && !setupComplete ? <span className="footer-requirement" role="status">{setupRequirement}</span> : step === 1 && !imageStepComplete ? <span className="footer-requirement" role="status">{imageRequirement}</span> : ready ? "✓ Ready for the ListingKing extension" : listingId ? "🌿 Draft stored in Neon" : `Draft · ${count} items`}</span><div>{step > 0 && <button className="secondary" onClick={() => setStep(current => Math.max(0, current - 1))} disabled={makingReady}>Back</button>}{step < 4 ? step === 0 && !setupComplete ? <span className="continue-locked">Continue unlocks after required fields are complete.</span> : step === 1 && !imageStepComplete ? <span className="continue-locked">Upload front images to continue.</span> : <button className="primary" onClick={() => void next()} disabled={savingDraft}>{savingDraft ? "Saving draft…" : <>{footerLabel}</>}</button> : <button className="primary" onClick={() => void saveReady()} disabled={makingReady || ready}>{ready ? "✓ Listing ready" : makingReady ? "Making listing ready…" : "Save smart listing →"}</button>}</div></footer>
    </section>
  </main>;
}
...
function Setup({ count, setCount, templates, templateId, setTemplateId, loading, reload, details, update }: { count: number; setCount: (value: number) => void; templates: SavedTemplate[]; templateId: string; setTemplateId: (value: string) => void; loading: boolean; reload: () => Promise<void>; details: Details; update: (key: keyof Details, value: string) => void }) {
  return <div className="setup-layout">
    <div className="setup-left">
      <p className="eyebrow step-eyebrow">SETUP YOUR LISTING</p>
      <h1>Start with a template<br />built for Meesho</h1>

      <h3>Choose a reusable template</h3>
      <div className="template-choice">
        ...
        {templates.map(template => <button type="button" key={template.id} className={`template ${template.id === templateId ? "selected" : ""}`} onClick={() => setTemplateId(template.id)}><b>MS</b><span><strong>{template.name}</strong><small>{template.categoryLabel} · v{template.version}</small></span>{template.id === templateId && <i>✓</i>}</button>)}
      </div>
      <button type="button" className="template create" onClick={() => void reload()}><b>↻</b><span><strong>Refresh templates</strong><small>Load newly captured Meesho templates</small></span><span className="refresh-arrow">›</span></button>

      <h3>How many catalog items?</h3>
      <div className="count-row">{[5, 10, 25, 50].map(value => <button key={value} className={count === value ? "count selected" : "count"} onClick={() => setCount(value)}>{value}</button>)}<label className="custom-count">Custom <input type="number" min="1" max="50" value={count} onChange={event => setCount(Math.max(1, Math.min(50, Number(event.target.value) || 1)))} /></label></div>
    ...
    <div className="setup-right">
      <div className="product-card">
        <div className="product-card-header"><h2>Product details</h2><span className="required-note"><span className="field-required">*</span> Required</span></div>
        <hr className="orange-divider" />

        <label className="form-label">SKU Code <span className="field-required">*</span>
          <small className="form-hint">Unique integer code (e.g. 4789)</small>
          <input type="number" min="1" max="99999" placeholder="e.g. 4789" value={details.skuCode} onChange={event => update("skuCode", event.target.value)} required aria-required="true" />
        </label>

        <div className="form-grid">
          <label>Product name <span className="field-required">*</span><input value={details.productName} onChange={event => update("productName", event.target.value)} placeholder="Enter product name" maxLength={80} required aria-required="true" /></label>
          <label>Product type / category <span className="field-required">*</span><input value={details.category} onChange={event => update("category", event.target.value)} placeholder="Select category" /></label>
        </div>

        <div className="form-section-label">Specifications<small>Core attributes of your product</small></div>
        <div className="form-grid">
          <label>Material / fabric<input value={details.material} onChange={event => update("material", event.target.value)} placeholder="Select material" /></label>
          <label>Colour<input value={details.color} onChange={event => update("color", event.target.value)} placeholder="Select colour" /></label>
          <label>Fit, style or pattern<input value={details.style} onChange={event => update("style", event.target.value)} placeholder="Select fit, style or pattern" /></label>
          <label>Intended audience<input value={details.audience} onChange={event => update("audience", event.target.value)} placeholder="Select audience" /></label>
        </div>

        <div className="form-section-label">Highlights &amp; search<small>Help Meesho understand your product better</small></div>
        <label className="form-label wide">Key features <span className="field-required">*</span><textarea value={details.features} onChange={event => update("features", event.target.value)} placeholder="Enter key features" maxLength={800} required aria-required="true" /><span className="char-count">{details.features.length} / 800</span></label>
        <label className="form-label wide">Optional search keywords<small className="form-hint">Add keywords separated by commas</small><input value={details.keywords} onChange={event => update("keywords", event.target.value)} placeholder="Add keywords separated by commas" /></label>
        <label className="form-label wide">Notes for content review<small className="form-hint">Anything our team should know?</small><textarea value={details.notes} onChange={event => update("notes", event.target.value)} placeholder="Add notes (optional)" maxLength={500} /><span className="char-count">{details.notes.length} / 500</span></label>
      </div>
    </div>
  </div>;
}
...
function Skus({ listingId, onError }: { listingId: string; onError: (message: string) => void }) {
  ...
  return <><div className="section-title"><div><p className="eyebrow step-eyebrow">STEP 04</p><h2>Every item receives a unique SKU</h2><p>These exact stored IDs are used again on Review and by the ListingKing extension.</p></div></div><div className="sku-list">{items.map(item => <label key={item.id}><span>{String(item.position).padStart(2, "0")}</span>{editingId === item.id ? <input value={editValue} onChange={event => setEditValue(event.target.value)} autoFocus /> : <input value={item.sku || "SKU pending"} readOnly />}{editingId === item.id ? <span className="sku-actions"><button className="primary" onClick={() => void saveEdit(item.id)} disabled={saving}>{saving ? "…" : "Save"}</button><button className="secondary" onClick={cancelEdit} disabled={saving}>✕</button></span> : <span className="sku-actions"><button className="secondary" onClick={() => startEdit(item)}>Edit</button><em>● Available</em></span>}</label>)}</div></>;
}
